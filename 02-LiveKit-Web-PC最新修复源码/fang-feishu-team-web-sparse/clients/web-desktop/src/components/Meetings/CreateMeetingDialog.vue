    ...
    <transition name="dialog" appear>
      <div v-if="modelValue" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40"
        ...
        <div class="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-[440px] p-6">
          ...
          <p class="text-sm text-gray-500 mb-4">每个账号可创建独立会议，系统会自动分配 LiveKit 安全房间。</p>
          ...
          <p class="mt-1 text-xs text-gray-400">留空时将使用默认会议名称。</p>
...
async function onCreate() {
  ...
  try {
    ...
    const m = await meetingStore.create(normalizedTitle ? { title: normalizedTitle } : {})
    emit('created', m)
    onClose()
  } catch (e) {
