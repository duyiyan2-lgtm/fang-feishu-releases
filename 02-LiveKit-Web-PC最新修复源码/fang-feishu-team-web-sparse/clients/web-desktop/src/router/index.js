...
// Web 端保留干净 URL；Electron 的 file:// 页面使用 hash，刷新子页面不会白屏。
const history = window.electronAPI?.isElectron ? createWebHashHistory() : createWebHistory()
const router = createRouter({ history, routes })

router.beforeEach((to) => {
  const userStore = useUserStore()
  if (to.meta.public) return userStore.isLoggedIn ? '/messages' : true
  return userStore.isLoggedIn ? true : '/login'
})

router.afterEach((to) => { if (to.meta?.title) document.title = `${to.meta.title} · Feishu-like Workspace` })
