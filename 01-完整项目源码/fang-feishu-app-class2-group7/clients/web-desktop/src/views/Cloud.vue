<template>
  <div class="flex flex-col h-full bg-[#F7F8FA] dark:bg-[#12151C] transition-colors duration-200">
    <!-- 工具栏 -->
    <div class="h-13 px-5 flex items-center justify-between border-b border-line dark:border-gray-700/80 bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm flex-shrink-0">
      <div class="flex items-center gap-1.5 text-sm text-ink-secondary dark:text-gray-300">
        <div class="w-8 h-8 rounded-lg bg-primary-soft dark:bg-primary/15 flex items-center justify-center">
          <FolderIcon class="w-4 h-4 text-primary" />
        </div>
        <span class="hover:text-primary cursor-pointer transition-colors font-medium">我的空间</span>
        <ChevronRightIcon class="w-3.5 h-3.5 text-ink-tertiary" />
        <span class="text-ink dark:text-gray-100 font-semibold">根目录</span>
      </div>
      <div class="flex items-center gap-2">
        ...
        <button
          ...
          :class="view === 'trash' ? '!border-primary !text-primary !bg-primary-soft' : ''"
          @click="view = view === 'trash' ? 'list' : 'trash'; if (view === 'trash') loadTrash()"
        >
          <TrashIcon class="w-4 h-4" />回收站
        </button>
        <button
          v-if="view === 'list'"
          type="button"
        ...
        <button type="button" class="ff-btn-primary" @click="openCreate">
          <PlusIcon class="w-4 h-4" />新建
        </button>
      </div>
    </div>

    <!-- 文件网格 -->
    <div class="flex-1 overflow-y-auto scrollbar-auto px-5 py-5">
      <div v-if="loading" class="text-center py-16 text-sm text-ink-tertiary">
      ...
      <template v-else-if="view === 'list'">
        <div v-if="files.length === 0" class="text-center py-16">
          <div class="w-16 h-16 mx-auto rounded-2xl bg-primary-soft dark:bg-primary/10 flex items-center justify-center mb-3">
            <CloudIcon class="w-8 h-8 text-primary/50" />
          </div>
          <p class="text-sm text-ink-tertiary">还没有文件，点击「上传」开始</p>
        </div>
        <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
          <div
            v-for="file in files"
            :key="file.id"
            class="group relative bg-white dark:bg-gray-900 border border-line dark:border-gray-700/80 rounded-xl p-3
                   hover:shadow-card hover:border-primary/35 transition-all duration-150"
          >
            <div
              class="aspect-square rounded-lg flex items-center justify-center mb-2 relative cursor-pointer"
              :style="{ background: file.color + '15' }"
              @click="preview(file)"
            >
              <component :is="fileTypeIcon(file.type)" class="w-14 h-14" :style="{ color: file.color }" />
              <span
                class="absolute bottom-1.5 right-1.5 text-[10px] uppercase font-semibold px-1.5 py-0.5 rounded-md text-white shadow-sm"
                :style="{ background: file.color }"
              >{{ file.type }}</span>
            </div>
            <h4 class="text-sm font-medium text-ink dark:text-gray-100 truncate text-center" :title="file.name">{{ file.name }}</h4>
            <div class="text-center text-2xs text-ink-tertiary mt-1">{{ file.size }}</div>

            <div class="absolute top-2 left-2 right-2 flex justify-between opacity-0 group-hover:opacity-100 transition-opacity">
              <button type="button" class="w-7 h-7 rounded-lg bg-white dark:bg-gray-800 shadow-soft hover:text-primary flex items-center justify-center" title="预览" @click.stop="preview(file)">
                <EyeIcon class="w-3.5 h-3.5" />
              </button>
              <button type="button" class="w-7 h-7 rounded-lg bg-white dark:bg-gray-800 shadow-soft hover:text-primary flex items-center justify-center" title="分享" @click.stop="openShare(file)">
                <ShareIcon class="w-3.5 h-3.5" />
              </button>
              <button type="button" class="w-7 h-7 rounded-lg bg-white dark:bg-gray-800 shadow-soft hover:text-primary flex items-center justify-center" title="下载" @click.stop="download(file)">
                <ArrowDownTrayIcon class="w-3.5 h-3.5" />
              </button>
              <button type="button" class="w-7 h-7 rounded-lg bg-white dark:bg-gray-800 shadow-soft hover:text-red-500 flex items-center justify-center" title="删除" @click.stop="del(file)">
      ...
      <!-- 回收站视图 -->
      <template v-else>
        <div v-if="trashFiles.length === 0" class="text-center py-16">
          <TrashIcon class="w-12 h-12 mx-auto text-ink-tertiary/40 mb-2" />
          <p class="text-sm text-ink-tertiary">回收站是空的</p>
        </div>
        <div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
          <div
            v-for="file in trashFiles"
            :key="file.id"
            class="group relative bg-white dark:bg-gray-900 border border-line dark:border-gray-700 rounded-xl p-3 opacity-75 hover:opacity-100 transition-all"
          >
            <div class="aspect-square rounded-lg flex items-center justify-center mb-2 relative" :style="{ background: file.color + '15' }">
              <component :is="fileTypeIcon(file.type)" class="w-14 h-14" :style="{ color: file.color }" />
            </div>
            <h4 class="text-sm font-medium text-ink dark:text-gray-100 truncate text-center" :title="file.name">{{ file.name }}</h4>
            <div class="text-center text-2xs text-ink-tertiary mt-1">{{ file.size }}</div>
            <div class="absolute top-2 left-2 right-2 flex justify-between opacity-0 group-hover:opacity-100 transition-opacity">
              <button type="button" class="w-7 h-7 rounded-lg bg-white dark:bg-gray-800 shadow-soft hover:text-primary flex items-center justify-center" title="还原" @click.stop="onRestore(file)">
                <ArrowPathIcon class="w-3.5 h-3.5" />
              </button>
              <button type="button" class="w-7 h-7 rounded-lg bg-white dark:bg-gray-800 shadow-soft hover:text-red-500 flex items-center justify-center" title="彻底删除" @click.stop="onPermanentDelete(file)">
    ...
    <!-- 存储条 -->
    <div class="border-t border-line dark:border-gray-700/80 px-5 py-3 bg-white dark:bg-gray-900 flex-shrink-0">
      <div class="flex items-center justify-between mb-1.5">
        <span class="text-xs text-ink-tertiary">
          已使用 <span class="font-semibold text-ink dark:text-gray-200">{{ totalSizeText }}</span> · 共 {{ files.length }} 个文件
        </span>
        <span class="text-xs text-primary cursor-pointer hover:underline font-medium">升级容量</span>
      </div>
      <div class="w-full h-1.5 bg-surface-tertiary dark:bg-gray-700 rounded-full overflow-hidden">
        <div class="h-full bg-gradient-to-r from-primary to-violet-500 rounded-full transition-all duration-300" :style="{ width: usagePercent + '%' }" />
      </div>
    </div>
